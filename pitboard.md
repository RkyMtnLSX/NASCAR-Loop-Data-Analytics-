...
| #118 | IMPLEMENT the equipment/car prior (Archive C 2026-07-09, VALIDATED: thin-driver corr 0.433→0.518 + ride-change delta k=0.25). Data half DONE 2026-07-09: `backfill_loop_car_numbers_rr.sql` RUN by user, coverage verified cup 97.6 / oreilly 97.7 / trucks 93.0 pct (13,005 rows: 12,861 finish-verified + 144 trusted DQ-revision rows; 100.00% agreement with independent GFS source on 12,473 overlapping rows). Current-weekend cars come from entry_list.car_number — populated only when the user loads the entry list, so the equipment panel needs a "load entry list" empty state. Code half SHIPPED 2026-07-09 — stage 1 (b24d7beb): blend + ride-change delta live in buildSpeedScores, fully guarded; stage 2 (f851e3cb): Equipment-prior admin panel (thin + ride-change rows, load-entry-list empty state). First live board audited: Eckes/Annunziata/Honeycutt/Kligerman all explained (Archive C); de-meaned + own-excluded pool variants tested and REJECTED (raw pools win — seat assignment is signal). ONLY REMAINING: loop-loader car_number stamping on new race loads. |
