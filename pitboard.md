...
| #119 | ALLOCATION-CONDITIONED practice input — REVISED after ground truth (Archive C 2026-07-10): fresh-set DETECTOR IS DEAD as a session classifier (false pos: Martinsville/Coronado 2026 track evolution; false neg: Dover 2026 two-sets-allowed-unused). `practice_sessions.tire_sets` column added (ALLOWED sets, manual ground truth from entry blanks; 2026 cup stamped, 2024-25 NULL pending). Conditioned finding (filt103/best5 win on multi-set) is UNVERIFIED — only Chicagoland 2026 is a verified multi-set race. NEXT: (a) add Tire Sets input to practice uploader; (b) detector demoted to per-driver display hint on report card; (c) market test waits for verified multi-set sample (retrieve 2024-25 allocations or accumulate future ones). practice_laps names normalized ✓ 2026-07-10. |
