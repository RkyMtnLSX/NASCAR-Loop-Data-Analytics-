...
`driver_name`, `start_position`, `finish_position`, `mid_race_position`, `high_position`,
...
| #118 | IMPLEMENT the equipment/car prior (Archive C 2026-07-09, VALIDATED: thin-driver corr 0.433→0.518 + ride-change delta k=0.25). Data half: run `backfill_loop_car_numbers_rr.sql` (in Downloads — the RACING REFERENCE build, supersedes any earlier GFS-only file; 13,005 rows: 12,861 finish-position-verified + 144 trusted DQ-revision rows, PLUS 100.00% car agreement with the independent GFS source on all 12,473 overlapping rows, zero disagreements) so the sim reads car_number straight off loop_data. Current-weekend cars come from entry_list.car_number — populated only when the user loads the entry list, so the equipment panel needs a "load entry list" empty state. Code half: blend car-pooled rating when nCorrRaces < 4 + k=0.25 ride-change delta for established drivers; current car from entry_list.car_number; admin panel per UI spec. LOADER GAP: new race loads won't stamp car_number until the loop loader is updated — include in scope. Verify Reaume/Tyrrell differentiate. |
