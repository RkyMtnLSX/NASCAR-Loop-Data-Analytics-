...
- v6.3-st session-time correction: ledger 1-1 (wk1 trucks Richmond +.026; wk2 cup Richmond -.099). PROTOCOL CORRECTED 2026-08-23: the old "2 consecutive losses" revert trigger was a COUNTDOWN (fires after ~6 weekends even if the feature is neutral; ~14 even at a 70% win rate) and is REPLACED. New rule: emergency revert only on a single-weekend loss worse than 0.15 rho; no streak revert; verdict deferred to the pre-committed 8-10 timestamped sessions and judged on POOLED EFFECT SIZE, not W/L. Week 3 (cup NH R25) is pre-registered with both cards frozen pre-race and counts as a DATA POINT, not a verdict. BLOCKER: truck sessions keep uploading with no captured_at (Richmond, NH) so the pool accrues at half rate - fix the watcher for truck practice.
- DFS replay ledger (GPP ceiling vs mean, official FPTS, real fields): 5 races - GPP 4 wins, 1 tie. R5 trucks NH R18: GPP 175.6 vs mean 141.4 (GPP +34) but BOTH under the 196.25 median (~1512 and ~1945 of 2378). NEW + IMPORTANT: DK salary Spearman .384 and field ownership .369 both BEAT our projections at .291 - first time the market out-predicted us on a slate; and proj-per-$1k value correlated -0.346 with actual, i.e. the cap tilted the optimizer INTO the busts. n=1 slate (SE ~.17) - watch whether value inversion repeats.
- Ownership ground truth: 6 contests banked (Iowa cup+ore, Richmond trucks+cup, NH trucks). Refit at 8-10.
