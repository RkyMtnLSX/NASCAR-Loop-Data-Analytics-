# PITBOARD STATE
**2026-09-03 — INT DOMINANCE v2 SHIPPED (pre-registered, holdout-passed, BACKTEST_LOG same date).** Operator flagged the Darlington board's laps-led projections (top value 33 of 367). Cause: dominance was dealt by realized finish rank, so the top-LL car in every draw was the winner (real INT: it is NOT the winner 61% of the time) and the per-draw top share was capped at 32-37% for every intermediate (real top-car share 40%, Darlington 58%); fastest laps were dealt for every lap (real: 78% of laps are green). Fix at INT only: strength-keyed order + sorted-share curves + green-lap FL budget. Holdout 2025-26 cup INT: LL MAE 9.72->8.54, FL MAE 5.70->4.80, DK rho .311->.320, tier-1 LL bias +44.5->+23.7; win/top5/top10 unchanged. **OPERATOR ACTION: re-run + republish the Darlington cup (and O'Reilly) boards.** Open from it: tier-1 still under by ~24 laps on the two most concentrated seasons in the corpus (refit on all 62 at season end); tier 4-6 now slightly over; SHORT/ROAD/SS still deal a fastest lap for every lap (one-constant entry each, unmeasured); arm C (per-draw bootstrap of real share vectors — the 80% nights) is in the engine OFF behind domBoot for a ceiling-targeted registration. Cup evidence only; O'Reilly/trucks INT boards inherit, judged forward. **ALSO SHIPPED 2026-09-03 — START PROJECTION v4 (cup): trail10 + recent-form / Jayski-order term.** Holdout 2026 start MAE INT 7.56->6.24 (8/8), SHORT 7.93->7.56, SS 10.03->9.46, ROAD 0 by design; sim rail and favourite-gap pass; the pooled 55%-of-races gate landed one race short because road-course ties counted against it — operator shipped it as registered with that disclosed. O'Reilly added 2026-09-05 with its own fit (holdout 6.00->5.73, INT 7/7). The Jayski PDF panel now feeds the race sim (it only fed Qualifying Center before). CORRECTED: the feed's `qualifying_order` is car-number order, not the run order — the 56 stored races are junk for that purpose; do not backfill it. **OPERATOR ACTION: re-run + republish Darlington (order already loaded).** DATA FIX 2026-09-03: practice_sessions.best5 backfilled (3,267 rows) — it had been NULL on every pre-07-16 upload, so most 2026 cup boards and the whole reconstruction ran on overall_avg. QUEUED from the practice audit: (a) regenerate holdout-practice.txt carrying best5; (b) registered practice-weight sweep BY TRACK GROUP (practice partial signal INT +0.225 vs SHORT +0.138 at the same 0.15 weight) with a cup favourite-calibration gate (08-24 n=12 over-confidence never followed up). Also this session: Fastest Laps page cleanup (alignment, car-art state bug, Lap #/Start/Finish/Car # columns). 2026-09-05: O'Reilly Darlington entry list had SPONSORS in `organization` (positional paste) — data repaired from prior rows, bulk importer now header-aware / known-team matched / stops on a sponsor-looking column (pitboard.md same date). Same day: DK split its driver markets across two pages (Top 5 alone) — DK box parser now segment-aware (`parseDkPages`, fixture-tested); paste every DK page into the one box. 2026-09-12: Jayski moved the PDF team column (Owner, after Sponsor) — parsePdf now picks the team by known-team match / header position, not slot (d79d3b7). **SIM BUG FIXED 2026-09-12: normalizeName was half commented-out since 09-05 (f291ae2) — marked (C)/(P) drivers ran without practice, aliases dead, accents unstripped on every board 09-05→09-12; re-run + republish this weekend's boards (pitboard.md 2026-09-12).** DFS 2026-09-12: Lineup Optimizer is CONTEST-FILE-FIRST — load the DK entries file, tick contests (GPP/Cash), Build for these contests, Fill; legs sized per contest (`wants`). The 'Operator' build preset was registered and CLOSED 09-13 (fails). PRODUCT RULE (operator, 2026-09-13): no user-uploaded data is stored in the database - DK entries files, lineups and fills are processed in the browser only; presets are fixed rule sets in code, never learned per user. PINNED: scope a Jayski historical qualifying-order archive (2025-26 cup + O'Reilly) so ARM O can be backtested on real orders instead of the last-finish proxy.
Volatile snapshot — REPLACE on change (git history is the archive). Updated: 2026-09-05 (DFS post-board rule, beta access). **LAUNCH IS PUSHED TO NEXT SEASON (operator, 2026-08-31) — the Chase starts Sunday and we are not ready.** The sim now runs OUTSIDE the browser: src/lib/simEngine.js + scripts/, see scripts/README.md. Nine model studies were registered and run this session; the DNF-tilt line is CLOSED. **TWO changes shipped: the SS caution pin, and the cliff fix — the caution buttons no longer move attrition, `dnfRate` is now the only attrition dial (operator-approved).** Both remaining blockers are the same blocker — one season of accumulated data. PIT DATA BLOCKER IS CLOSED (verified against the database 2026-08-31, not against a doc): `pit_stops` holds 81,647 rows / 413 races / 2022-2026, last loaded 2026-08-30. The 24 races still without pit rows are tracks where NASCAR publishes no pit timing at all — Bristol dirt, Lime Rock, IRP, Portland, Road America, Rockingham; 17 of the 24 are trucks. Nothing to backfill. pitcrewrank.com is retired and its table dropped; pit crew is live via `pit_stops` (`pitCrew: 0.06` in every weight set).

## 2026-09-05 — DFS POST-BOARD RULE + BETA TESTER ACCESS (`5913515`, `4ddc283`, this commit)

- **DFS Center was serving PRE boards.** It loaded the newest `sim_results` row with no stage
  filter; both Darlington Cup R27 and O'Reilly R25 boards this week are `stage='pre'` (projected
  starts). Owner rule: the optimizer must never build on projected starting positions. Now: newest
  board of any stage names the race, only that race's `post` board is loaded, else a blocked state
  naming the pre board. First pass (`5913515`) would have served last week's Daytona post board —
  caught and fixed in `4ddc283` before the operator refreshed. `dfs_sim_samples` gained `stage`
  (migration `dfs_sim_samples_add_stage`, 13 rows backfilled: 11 post / 2 pre); publish replaces
...
- **Beta testers** (operator choice: admin-panel tool, default through end of 2026 season).
  ...
  the confirmation email before sign-in; the panel flags unconfirmed accounts.
- Earlier same day: four DFS review fixes (`5f9e22c`) — build-token cancellation, salary
  insert-then-delete, replay grades with official FPTS + contest_type filter, surfaced read errors.
...
- **Ledger 2026-09-18 (Trucks Bristol R19, N=20):** GPP 8 / cash 3 / tie 2; Portfolio row 2 / 2 (77.5 vs 88.7, rules OFF - trucks stay off). GPP best-of-20 324.05 (~19th of 845) on Garcia's win from P12; cash on the median after Riggs pole -> P32 at 56% owned. rho model .325 first of three, third week running. DfsReplay page REPAIRED 09-18 (legacy-schema fallbacks) - it was silently broken since the 09-14 ship because the SQL is still not run; run sql/dfs_operator_entries.sql then re-save Bristol for the Operator column.
- **[OPEN 2026-09-17] E[max] set saturates near ~60 lineups:** the selector stops when no candidate improves any sampled draw, so entries above that point add nothing and the product under-fills 100+ entry contests at 100% max exposure (topUpLineups skips). Continuation criterion or more draws - register before changing (pitboard.md 09-17).
- **[CLOSED 2026-09-13] 'Operator' build preset — registered (rule 3 as a mechanical gate: floor 0%, low-projected-own punts up to 55%, >= 1 punt per lineup) and FAILED on the 10-race replay harness: prize 26.1 -> 16.2 (-38%), 2/8 vs rules-on, below the null arm. Legs starve under tier-two minimums and the projected-ownership gate is toothless (proj own is monotone in our projection). Constant stays unexposed in dfsPortfolio.js; BACKTEST_LOG 09-13. The 09-05 evidence for rules 2 + 4 stands (shipped, +14%). Original item: Operator construction beat both solvers at Darlington O'Reilly (1st of 1,189 vs product GPP ~268th, cash ~1,041st) on a board with rho .35.** Exposure table + implied rules in BACKTEST_LOG same date: this was 1 of 3 contests x 20 - the other two (Allgaier 95% / 85%, solver-like builds) finished BELOW the field median (mean pctile 38 / 36); only the hand-built set won (mean pctile 86); tier-two studs 50-80%, 1-2 mid-priced sub-10%-owned punts (never floor cars), full cap. PROPOSED 'Operator' build preset + weekly replay row so the ledger scores it (n=1 today). Replay ledger now GPP 4 / cash 3 / tie 2.
...
- **[SHIPPED 2026-09-07] Per-car laps-down penalty — O'Reilly + Trucks (LAP_PENALTY 0.15); cup OFF.**
  ...
  frozen, scored on 2025-26 (94 races): rho .5397 -> .5418 (56/38), Brier 53/37, win logloss 67/26,
  top-5 64/30 - the only change this weekend to move FINISH ORDER on held-out years. Trucks rho 20/9,
  O'Reilly Brier 17/8 / win 20/5. Cup FAILED (P26+ residual away from zero, slowest-quarter calibration
  crosses below actual). Stacks on the asymmetric noise. config.lapFeature on boards.
- **[RE-SCOPED 2026-09-07] The "cup back-of-field problem" is a BOTTOM-FIVE problem.** On 2025-26, cup
  P26-34 starters finish ~1 position BETTER than projected; only P35-40 are over-projected (+0.9).
...
- **[SHIPPED 2026-09-06] Projected ownership v3: one top-end hinge term.** The rank-only curve was
  ...
  tie (6.349 -> 6.350, 5/3), top-3 chalk error 12.1 -> 11.5 (7/2); operator shipped it on the tie.
  Blaney Iowa 43 -> 51 (actual 72) - half the gap; the other half is the crowd liking drivers we
  do not rank up top (Hill ore Iowa 43.5% at z = 0.4) and no shape fixes that. Crowd signal that is
  real: PRACTICE (+0.85), not the pole. Consumers: Proj Own% column, Portfolio chalk definition,
  Replay portfolio field model (now the same function). Not leverage. BACKTEST_LOG 09-06.
...
1. [CLOSED 2026-09-06] Payout-aware (duplication-weighted) GPP objective: TESTED, LOSES to E[max]
   ...
   sampled from ownership score far below real entrants, so the objective buys floor, not ceiling.
   Needs real contest lineups to work; DK does not publish them. Do not re-open on a new payout
   curve. E[max] stays. Realised ROI of the product's 20-lineup sets on a DK-like curve across the
   9 replay races: about -68%. What is left for DFS is CONSTRUCTION ACROSS CONTESTS (portfolio
   builder + chalk-stance schedule, Operator preset) - BACKTEST_LOG 09-05 / 09-06.
   (Original 08-30 note follows for history.)
