...
- [FOLLOW-ON, same day] THE BOARD READS PACE, NOT FINISH. Operator corrected my Byron example (he lost a wheel running top 5 - 26 laps led, high pos 1, 296/301 - so the 14.3% post rating was right on speed and killed by attrition). Retested properly: in 9 of 9 races the board's win pct correlates better with AVG RUNNING POSITION than with FINISH, gap +0.070 to +0.430, mean +0.181 - and the gap WIDENS pre->post (+0.113 -> +0.181). In cup the pace read improves 4/4 (rho .525 -> .610) while the finish read does not move (.478 -> .466). Practice buys pace knowledge; the pace-to-finish conversion throws most of it away. REVISES the diagnosis behind the cup win-market gap: not over-confident weights, but unmodelled attrition / caution timing / track position. Next aim is the CONVERSION layer (queue 8 DNF tiering), not another weight sweep. Cheap first cut: score proj_finish against avg_position across the full board archive.
