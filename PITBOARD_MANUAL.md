...
- practice_laps: captured_at timestamps exist from 2026-08-14 forward (LAPS_RAW sheets). Practice grading happens AT UPLOAD TIME (Admin.js) — a card only regrades on re-upload. CONSEQUENCE, fixed 2026-09-03: `practice_sessions.best5` (the sim's cup/trucks practice input) was NULL on every session uploaded before the 07-16 best5 ship (~70% of 2026 cup boards silently ran on overall_avg). Backfilled from practice_laps with the grader's exact formula (mean of the 5 fastest laps in (10s,1200s); verified 440/440 against already-populated rows), 3,267 rows. Seven sessions have no lap rows at all and stay NULL. Any future column added to the grader needs the same backfill or the sim runs on the fallback without saying so.
- Stints (grader `parseStints`, 2026-09-05): a stint ends at a numbering gap OR at any lap > 1.2x the driver's median (pit / in-out lap, dropped). Needed because watcher LAPS_RAW keeps every lap under 300s, so short-track pit visits arrive as numbered 40-300s laps, not gaps (Darlington O'Reilly: 40 such laps; cup NH R25 in the DB with 10 — its card was graded before this rule and regrades only on re-upload). Verified against the old-method sheet: 1,464/1,472 shared laps agree to rounding; the 8 exceptions are 4 feed-stall pairs where the watcher split the elapsed time evenly across two laps (sums exact).
- Practice upload race-# guard reads the `races` registry, which only held COMPLETED races: a return visit (Darlington O'Reilly R25 after R6) was 'corrected' to the spring number. Fixed 2026-09-05: R# past the registry's last race with no race under that number = return visit, offers a stub for the entered number; the guard's resolved number is now the one written (setState was async, so 'upload as R6' wrote R25 anyway). The rest of the 2026 O'Reilly (R25-R33) and Trucks (R19-R25) schedule is seeded as stub rows from cf.nascar.com/cacher/2026/{2,3}/race_list_basic.json (nascar_race_id + scheduled_laps set); cup has only R27 Darlington seeded (the cup feed is too large for WebFetch — seed the rest from the operator's machine). Loaders adopt stubs by series/year/track/R#, so post-race loads fill these rows rather than duplicating.
...
- Practice grader v6.4-sets (2026-09-05): when `tire_sets` = 1 (uploader field, defaults from `tire_allocations`), tire age is cumulative laps across ALL stints (gate: 36 labeled 1-set cup sessions, rho vs driver rating .503->.531, W19/L12). When tire_sets >= 2 or null, the legacy reset-per-stint proxy stays — the set-change detector LOST its gate on 7 labeled multi-set sessions (.583 legacy vs .534) and is display-only (runs-by-set in the Laps column). Rejected same day: concave age, z-score composite, per-driver falloff slope (BACKTEST_LOG). Report card shows runs-by-set in the Laps column.
...
- Per-car laps-down penalty (2026-09-07): O'Reilly and Trucks ONLY. SimulationCenter computes each driver's `lappedRate` from own-series loop_data (recency-weighted 0.85 per race back, running finishers only, laps_completed below the race maximum = lapped, at least 3 prior races) and buildSpeedScores subtracts 0.15 x (rate - field median) x 100 from the speed score (`LAP_PENALTY`; `__lapPen` on the row). Fitted on 2022-24, frozen, scored on 2025-26: finish rho .5397 -> .5418, Brier 53/37, win log-loss 67/26. Cup failed the held-out test (its P26-34 already finish better than projected; only the last five cars are rich); since the floored-ceiling ship cup boards DO carry lappedRate, and the penalty is switched off for cup through the opts argument instead. Published boards carry `config.lapFeature`. Backtest boards have no lappedRate unless the harness attaches it. BACKTEST_LOG 2026-09-07.
- Per-car upside ceiling with floor (2026-09-07, later): ALL series. In runRaceSim (`simConfig.carCeilFloor`, always set by SimulationCenter; `config.carCeilFloor` 'v1-0.70' on boards) a driver whose `lappedRate` is above 0.70 keeps only (1 - rate) of an UPSIDE noise draw (floor 0.1x); downside untouched, no mean shift, every car at or under 0.70 untouched. SimulationCenter now computes `lappedRate` for every series (it was O'Reilly / trucks only) and passes `buildSpeedScores(drivers, weights, { lapPenalty: series !== 'cup' })` so the laps-down MEAN penalty above still applies only where it was measured. 256-race result: cup rho / Brier tie, P35-40 residual +1.48 -> +1.24 with P26-34 unchanged (the un-floored form, all rates, failed cup by taking upside from P26-34 cars who finish better than projected); O'Reilly and trucks rho better, Brier tie / better, stacks on the asymmetric noise. It is the fix for the Finchum-class car (13 cup starts, 7 lapped finishes, 5 DNFs, a top-20 sim ceiling) and moves nothing else. BACKTEST_LOG 2026-09-07.
- Asymmetric finish noise (2026-09-07): in runRaceSim, O'Reilly and Trucks at intermediate / short ovals ONLY (`simConfig.asymNoise`, set by SimulationCenter; published in `config.asymNoise` as 'v1-upside-0.5' / 'off'). Below-median cars keep only (0.5 + speed percentile) of an UPSIDE noise draw; downside and every car at/above median untouched, so the favourite's distribution cannot change. Why: the back of the field was projected ~1 position too well (P26+ residual +1.04 on 256 races) because one symmetric width let a P36 car land mid-pack. 256-race result: finish rho tie, top-10 Brier .1516->.1512, top-5 log-loss 197/58, P26+ residual +1.04->+0.68. Trucks passed every guard; O'Reilly passed Brier/top-5 with win log-loss inside noise; CUP LOSES Brier (38/58 at n=101 - a cup field's slower half genuinely carries upside) so cup stays symmetric; superspeedways and road courses lose under every noise form and are excluded. Backtest scripts run with the flag OFF unless they pass it. BACKTEST_LOG 2026-09-07.
- corrHistory driver rating (2026-09-05): correlation-GROUP rows only, SHRUNK toward the driver's all-track own-series rating with prior weight k=4 (yrWt units); no-group-row drivers enter at the pooled rating. 427-race loop-data test: beats pure group in every series (cup .632->.639, O'Reilly .777->.789, trucks .749->.766), largest at superspeedways; cup road courses tie. Operator-directed ship without a sim A/B; replay ledger judges forward. BACKTEST_LOG 2026-09-05.
- Projected starts (pre-qualifying boards): trail10 (mean of last-10 start pctiles, hybrid SS/ROAD conditioning, 1..K re-rank, per-sim sampling) PLUS, for CUP since 2026-09-03 (trail10-v4-form, pre-registered + holdout-passed), a recent-form term: beta_g x (x - 0.5) with x = the Jayski qualifying order for this race when loaded (Admin PDF panel -> qualifying_results.draw_order; later = better) else previous-round finish pctile. beta cup INT .25 / SHORT .16 / SS .19 / ROAD 0; O'Reilly (added 2026-09-05, same metric per operator, own fit + holdout) INT .17 / SHORT .14 / SS .04 / ROAD 0. Why: since 2025 the Cup order is a metric (70% last finish, 30% owner points, worst first) and it moved qualifying predictability off the ten-race average. NOTE the NASCAR feed's `qualifying_order` column is car-number sort order, NOT the run order — never use it. Trucks stay on v3.5 (unmeasured).
- Dominance (laps led / fastest laps, DFS): at INTERMEDIATES (int-dom-v2, 2026-09-03, pre-registered + holdout-passed) dealt by a STRENGTH order (speedScore + half the draw's finish noise + independent noise; k_LL .5, k_FL .75) from sorted-share curves, and fastest laps cover only the green-lap fraction (.779). Other groups still deal by finish rank from LL_CURVES_G/FL_CURVES_G and hand out a fastest lap for EVERY lap — known wrong, unmeasured, needs its own one-constant entry. Practice tilt mult-v1 (best5 pctile; FL is the more practice-driven target) sits on top and is a NO-OP until practice is loaded. Reconstruct the old allocator with domPool:'finish'. Harness: scripts/backtest-int-dominance.js.
...
- DFS optimizer serves POST boards ONLY (owner rule 2026-09-05): the newest board of any stage names the current race; only that race's `stage='post'` board (real qualified / DK-listed starts) is loaded — never a pre (projected-start) board and never an older race's post board; otherwise the page shows a blocked "publish the Post board" state. `dfs_sim_samples.stage` tags draws; only post draws feed GPP E[max]; SimulationCenter replaces only same-stage draw rows.
- GPP set builder under exposure caps (2026-09-06, BACKTEST_LOG V4): candidates are diversified whenever a driver's share of the candidate pool exceeds his cap (top 1,500 candidates without him are appended), so the E[max] selector fills the set itself instead of `topUpLineups`; top-up, when it still runs, caps floor-salary punts (PUNT in the Value column) at 25% unless the user set a per-driver max. The Cram-45% build was the top-up filler, not E[max].
- **Proj Own% (v3, 2026-09-06).** own = 600% x exp(2.2 x projection-rank percentile + 0.115 x max(0, z - 1)) / sum, z = within-slate z-score of projected DK points. The hinge term lifts only the top ~sixth of the slate so a runaway favourite reads chalkier than a coin-flip favourite (the rank-only curve gave the top driver ~37% every week; actual averages 52%). Fitted LORO on 9 races; MAE unchanged, chalk-level error down 7/2. Derived from our projection - a field model, never a leverage signal.
- **Portfolio mode (2026-09-06).** Lineup Optimizer (nav: DFS Center -> Lineup Optimizer; the page was called "DFS Center" until 09-06) -> Mode "Portfolio (N contests)" builds N legs on one board for N separate contests. **CONTEST-FILE-FIRST (2026-09-12, operator):** load the DK entries file (Lineups -> EDIT ENTRIES -> CSV) BEFORE building; each ticked GPP contest becomes a leg sized to its own entry count (`wants` per leg in dfsPortfolio.js, e.g. 40/20/20), cash contests (Double Up / 50-50 / H2H / Triple Up / Booster, auto-detected from the name, overridable) share one cash build, and "Fill selected contests" writes each contest from its own set. The Lineups / Legs inputs are driven by the file while a plan is loaded. The wrong-file trap names what was uploaded (entry-history export, contest-standings). Manual mode without a file is unchanged. RULE (operator, 2026-09-13): nothing a user uploads is stored server-side - the entries file is parsed with FileReader and the filled CSV is generated in the browser; the only DFS uploads that persist are the operator's own (salaries, contests, ownership). Build presets are fixed rule sets in code (the 'Operator' preset was registered and failed 09-13 - BACKTEST_LOG), never per-user data. Operator rules (on by default for Cup and O'Reilly, off for Trucks - backtested +14% / 7-2 on the 9 replay races, lost both truck races): tier-two studs (proj rank 3-8, $8,400-$10,000) 50-80% per leg, floor cars <= 10%, mid punts ($5,500-$6,200) <= 25%, at most 2 punts per lineup, salary >= $48,800, no lineup reused across legs, 60% cap on any driver across the whole portfolio. Chalk stance: drivers projected over 35% owned are capped at 50% per leg by default; the "0 / 50 / 50" option fades them entirely in leg 1 (finds a higher peak, returned zero in 5 of 9 backtest races - a deliberate high-variance mode). Your own per-driver min/max, locks and excludes override all of it. Legs are built round-robin (one pick per leg per round, portfolio cap live, no leg gets another's leftovers). When a leg is short the card names the binding constraint (portfolio cap on whom / tier-two minimum unreachable for whom / candidate pool exhausted) and offers rebuild buttons (portfolio cap 65 / 70%, drop the tier-two minimum for that leg) - it never pads a short leg with filler. Click a leg tab, Export CSV, upload to that contest; repeat per leg. The built set has a Hide / Show lineups toggle so the driver pool stays reachable. DFS Replay scores a Portfolio row every week (realised prize vs the plain set) - that ledger, not the backtest, decides whether trucks turn on and whether the schedule earns a default.
- DFS Center Value column (2026-09-05) = marginal DK pts per $1K ABOVE the salary floor: (projDK - floor projection) / (salary - floor salary). Floor cars (within $500 of the minimum) show PUNT and are compared on Proj DK. Replaced pts/$K, which ranked by salary (every floor car starting last projects ~24 and topped it weekly). pts/$K stays in the tooltip. Display only - the optimizer never used the column. Known related limitation: sim DNF spread (7-13%) is flat vs reality for P35-38 starters (23.5% DNF, avg finish 29.9 at O'Reilly short/INT 2025-26) so floor projections run ~4 DK pts rich; DNF-by-tier is a CLOSED line (STATE), not reopened.
- DFS optimizer: GPP mode default — candidates = per-draw exact optima + mean optimizer; the SET is chosen greedily to maximise E[max] (expected score of the best lineup in the delivered set) across the stored draws (2026-08-30; the earlier per-lineup p90 ranking is gone — this line was stale until 2026-09-12). Known limit: high-conviction boards collapse candidate diversity (mean ≡ GPP); V4 candidate diversification (09-06) is the answer, not a leverage overlay (closed). DK place-points helper runs ~1pt/driver hot midfield — always prefer official FPTS from contest standings files. Right DK file = contest-standings zip CSV, NOT the entry-history export.
...
Three tiers: anon = nothing (stale-sample landing TBD); authenticated subscriber = product-table read (blanket RLS policy); admins table = everything. **Beta testers (2026-09-05):** an ordinary account (sign up on /subscribe, no card) plus a `subscribers` row `plan='beta', status='active', access_until=<date>` — `has_access()` and `useSubscriber()` already honor `access_until > now()`, so no gate or flag changes. Managed from Admin → Beta Access (RPCs `grant_beta_access(email, until)` — 2026-09-06: `#variable_conflict use_column` added after the first real grant failed with "column reference user_id is ambiguous" (RETURNS TABLE names shadow the columns); migration `beta_access_grant_variable_conflict_fix`, `revoke_beta_access(uid)`, `list_beta_access()`, `list_unassigned_accounts()`; all re-check `is_admin()`, anon EXECUTE revoked). Account/Subscribe pages show beta wording and hide Manage billing for beta rows. Expired/revoked beta falls to the normal inactive → See plans state. Stripe sandbox verified end-to-end: $24.99/mo founding (list $34.99) + $9.99 7-day pass, no trials; webhook checks raw-body signature; env changes require a fresh deployment. Password auth fully removed from the app. The route paywall (PAYWALL_ENABLED flag in App.js) does NOT protect the REST API — the RLS lockdown is the real paywall.
