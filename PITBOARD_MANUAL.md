...
- Practice grader v6.4-sets (2026-09-05): tire age = cumulative laps on the assigned tire SET when `tire_sets` is known (uploader field, defaults from `tire_allocations`; K sets -> at most K-1 detected changes, change = laps 2-4 of new run beat old run's last 3 by >2% of median lap). Gate: 36 labeled 1-set cup sessions, rho vs driver rating .503->.531, W19/L12. Legacy reset-per-stint when tire_sets is null. Report card shows runs-by-set in the Laps column.
