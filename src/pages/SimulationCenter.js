...
function buildSpeedScores(drivers, weights) {
  ...
  const raceCraftScores   = normalizeArr(drivers.map(d => d.raceCraftPct),   false) // higher = better
  ...
  const w = {
    ...
    raceCraft:    weights.raceCraft    / wTotal,
    trackHistory: weights.trackHistory / wTotal,
  }
    ...
    async function load() {
      try {
        ...
        trackRows.forEach(r => {
          ...
          const yr     = parseInt(r.year) || 0
          if (name && fin > 0) {
            if (!trackByDriver[name]) trackByDriver[name] = []
            trackByDriver[name].push({ fin, rating: isNaN(rating) ? null : rating, yr })
          }
        })
        const trackAvgMap = new Map(
          Object.entries(trackByDriver).map(([name, rows]) => {
            const yrWt = yr => yr >= 2026 ? 2.0 : yr === 2025 ? 1.2 : yr === 2024 ? 1.0 : yr === 2023 ? 0.8 : 0.6
            const totalWt = rows.reduce((s, r) => s + yrWt(r.yr), 0)
            const avgFin = rows.reduce((s, r) => s + r.fin * yrWt(r.yr), 0) / totalWt
            const rRows  = rows.filter(r => r.rating != null)
        ...
        const drivers = driverSource
          .map(e => {
            ...
            return {
              ...
              trackAvgFinish: trackAvgMap.get(name)?.avg       ?? null,
              trackAvgRating: trackAvgMap.get(name)?.avgRating ?? null,
              nTrackRaces:    trackAvgMap.get(name)?.n         ?? 0,
            }
          })
          .filter(Boolean)
                  ...
                  {displayRows.map((row, ri) => {
                    ...
                    return (
                      <tr key={row.name} style={{ background: bg, borderBottom: '1px solid var(--border)' }}>
                        ...
                        {showBreakdown && (
                          <>
                            {['corr', 'lrp', 'srp', 'sp', 'fall', 'track'].map(k => (
