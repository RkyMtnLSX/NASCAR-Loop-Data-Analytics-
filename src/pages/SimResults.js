  ...
  return (
    <div style={{ marginTop: 28 }}>
      ...
      <div style={{ display: 'flex', gap: 16, alignItems: 'center', fontSize: 12, color: '#888', marginBottom: 10 }}>
        ...
        <span>sort <span style={{ border: '0.5px solid #333', borderRadius: 6, overflow: 'hidden', display: 'inline-flex' }}>{seg(mvSort, setMvSort, 'edge', 'Edge')}{seg(mvSort, setMvSort, 'best', 'Best')}{seg(mvSort, setMvSort, 'model', 'Model')}</span></span> <span style={{ marginLeft: 4 }}>bets <button onClick={function () { setMvQual(!mvQual); }} style={{ cursor: 'pointer', border: '0.5px solid #333', borderRadius: 6, padding: '2px 8px', background: mvQual ? '#123d24' : 'transparent', color: mvQual ? '#3fb950' : '#888', fontSize: 12 }}>Qualified only</button></span>{mvQual ? <span>min edge <input type="number" value={mvMinEdge} onChange={function (e) { setMvMinEdge(parseInt(e.target.value) || 0); }} style={{ width: 44, padding: '2px 4px', background: '#1a1a1a', border: '0.5px solid #333', borderRadius: 4, color: '#ddd', fontSize: 12 }} />%</span> : null}{mvQual ? <span>hide favs shorter than <input type="number" value={mvMaxFav} onChange={function (e) { setMvMaxFav(parseInt(e.target.value) || -100000); }} style={{ width: 52, padding: '2px 4px', background: '#1a1a1a', border: '0.5px solid #333', borderRadius: 4, color: '#ddd', fontSize: 12 }} /></span> : null}{mvQual ? <span style={{ marginLeft: 8, color: '#3fb950' }}>{rows.length} qualify</span> : null}
      </div>
