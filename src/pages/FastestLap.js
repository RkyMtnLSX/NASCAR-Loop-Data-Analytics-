        ...
        <table style={{borderCollapse:'collapse',width:'100%'}}>
          <thead>
            <tr>
              ...
              {hasMulti&&<th style={{...numHead,minWidth:52,fontWeight:700,color:'var(--accent)'}}>Avg</th>}
              {finalLabels.map(r=>(
                <th key={r.key} style={{...numHead,minWidth:44,fontSize:'0.65rem',fontWeight:600,writingMode:'vertical-rl',transform:'rotate(180deg)',height:90,verticalAlign:'bottom',padding:'6px 10px',whiteSpace:'nowrap',cursor:'default'}} title={r.name+' · '+r.date}>{r.label}</th>
              ))}
            </tr>
          </thead>
          <tbody>
