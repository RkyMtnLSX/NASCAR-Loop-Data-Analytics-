...
function useWeekendTrack(series, setter) {
  useEffect(() => {
    let cancelled = false
    supabase.from('featured_weekend').select('track_name').eq('series', series).maybeSingle()
      .then(({ data }) => { if (!cancelled && data && data.track_name) setter(data.track_name) })
...
function LoadNewRace() {
  ...
  useWeekendTrack(series, setSelTrack)
  const [tracks, setTracks] = useState([])
...
export default function Admin() {
  ...
  const __tabBar = (
    <div style={{ display: 'flex', gap: 4, borderBottom: '1px solid rgba(128,128,128,0.25)', marginBottom: 16 }}>
      {[['admin', 'Admin'], ['sim', 'Sim Admin'], ['grader', 'Sim Grader'], ['flags', 'Flagged Bets'], ['load', 'Load Data'], ['dfs', 'DFS']].map(t => (
