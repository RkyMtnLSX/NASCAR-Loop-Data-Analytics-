...
function EntryListManager() {
  ...
  const [jayskiLoading, setJayskiLoading] = React.useState(false)
  const [jayskiStatus, setJayskiStatus] = React.useState('')
  ...
  const fetchJayski = async () => {
    if (!jayskiUrl || !jayskiUrl.includes('jayski.com')) {
      setJayskiStatus('Enter a valid Jayski entry list page URL')
      return
    }
    ...
    try {
      ...
      setJayskiStatus('Error: ' + e.message)
    } finally {
      setJayskiLoading(false)
    }
  }
    ...
    <div className="card" style={{ marginBottom: 20 }}>
      ...
      <div style={{ marginBottom: 16, padding: '12px 14px', background: 'rgba(99,102,241,0.06)', border: '1px solid rgba(99,102,241,0.2)', borderRadius: 8 }}>
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
          <input
            value={jayskiUrl}
            onChange={e => setJayskiUrl(e.target.value)}
            placeholder="https://www.jayski.com/nascar-cup-series/2026-nascar-...-entry-list/"
            style={{ flex: 1, minWidth: 280, padding: '6px 10px', borderRadius: 5, border: '1px solid var(--border)', background: 'var(--bg-card)', color: 'var(--text-primary)', fontSize: '0.8rem' }}
          />
          <button onClick={fetchJayski} disabled={jayskiLoading} style={{ padding: '6px 16px', borderRadius: 5, border: 'none', background: 'var(--accent)', color: '#fff', fontWeight: 600, fontSize: '0.8rem', cursor: jayskiLoading ? 'wait' : 'pointer', opacity: jayskiLoading ? 0.7 : 1 }}>
            {jayskiLoading ? 'Fetching...' : 'Auto-Fill from Jayski'}
          </button>
        </div>
        {jayskiStatus && (
          <div style={{ marginTop: 7, fontSize: '0.78rem', color: jayskiStatus.startsWith('Error') ? '#f87171' : jayskiStatus.startsWith('Found') ? '#4ade80' : 'var(--text-muted)' }}>
            {jayskiStatus}
          </div>
        )}
