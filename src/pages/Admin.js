...
function LoadFastestLaps() {
  ...
  async function handleLoad() {
    if (!raceName || !raceDate || !track || !source.trim()) return
    setLoading(true)
    setStatus(null)
    try {
      ...
      rows.forEach(function(r) {
        const p = best[r.driver]
        if (!p || parseFloat(r.fastest_speed) > parseFloat(p.fastest_speed)) best[r.driver] = r
      })
      const drivers = Object.values(best)
      const res = await fetch('/api/load-fastest-laps', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ year: year, track_type: trackType, race_name: raceName, race_date: raceDate, track: track, rows: drivers })
      })
  ...
  const ready = !!(raceName && raceDate && track && source.trim())
  ...
  return (
    <div className="card" style={{ marginBottom: 20 }}>
      ...
      <div style={{ marginBottom: 14 }}>
        <label style={lbl}>Paste Lap Raptor Page Source</label>
        <textarea
          value={source}
      ...
      <button
        ...
        style={{
          ...
          background: ready && !loading ? 'var(--accent)' : 'var(--border)',
          color: ready && !loading ? '#000' : 'var(--text-muted)',
...
// Loop Data Audit Section
function LoopDataAudit() {
  ...
  return (
    <div className="card" style={{ marginBottom: 20 }}>
      <h2 style={{ fontSize: '0.9375rem', fontWeight: 600, marginBottom: 16 }}>Loop Data Audit</h2>
      ...
      {!loading && years.map(yr => (
        <div key={yr} style={{ marginBottom: 24 }}>
          <div style={{ fontSize: '0.75rem', fontWeight: 700, color: 'var(--text-secondary)', letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: 8 }}>
            {yr} &mdash; {byYear[yr].length} race{byYear[yr].length !== 1 ? 's' : ''}
          </div>
