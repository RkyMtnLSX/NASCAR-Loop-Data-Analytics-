...
function LoadNewRace() {
  ...
  const NAME_MAP = {
    ...
    'Baltazar Leguizamon':   'Baltazar LeguizamÃÂÃÂ³n',
    'Daniel Suarez':         'Daniel SuÃÂÃÂ¡rez',
  ...
  const NAME_LAST = [
    ...
    { key: 'Leguizamon',  val: 'Baltazar LeguizamÃÂÃÂ³n' },
    { key: 'SuÃÂÃÂ¡rez',      val: 'Daniel SuÃÂÃÂ¡rez' },
    { key: 'Suarez',      val: 'Daniel SuÃÂÃÂ¡rez' },
  ]
  ...
  return (
    <div className="card" style={{ marginBottom: 24 }}>
      ...
      <button onClick={handleLoad} disabled={loading} className="btn btn-primary" style={{ fontSize: '0.85rem' }}>
        {loading ? 'LoadingÃÂ¢ÃÂÃÂ¦' : 'Parse & Load'}
      </button>
      {status?.success && (
...
function LoadQualifying() {
  ...
  function handleTextChange(text) {
    ...
    if (text.trim()) {
      ...
      if (drivers.length === 0) setStatus({ type: 'error', msg: 'No qualifying rows found ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ make sure you Ctrl+A / Ctrl+C the full Racing Reference page' })
    } else {
      setPreview(null)
    }
  }
  ...
  return (
    <div className="card" style={{ marginBottom: 20 }}>
      ...
      <p style={{ fontSize: '0.8125rem', color: 'var(--text-muted)', marginBottom: 16 }}>
        ...
        {url && <> <a href={url} target="_blank" rel="noreferrer" style={{ color: 'var(--accent)', marginLeft: 6 }}>Open Racing Reference ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ</a></>}
      </p>
      ...
      {preview && preview.length > 0 && (
        <div style={{ marginBottom: 14 }}>
          <div style={{ fontSize: '0.8rem', color: '#22c55e', marginBottom: 8 }}>
            Parsed {preview.length} drivers ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Pole: {preview[0].driverName} ({preview[0].speed} mph)
          </div>
...
function LoadFastestLaps() {
  ...
  function parsePaste() {
    ...
    setParsed(rows)
    if (rows.length === 0) {
      setStatus({ type: 'error', msg: 'No rows parsed Ã¢ÂÂ go to Lap Raptor race page, Ctrl+A, Ctrl+C, paste here' })
    } else {
...
function SimFormulaPanel() {
  ...
  const factors = [
    ...
    ['Long Run Pace',  'overall_avg from practice_sessions Ã¢ÂÂ all laps across all stints, any lap >8% slower than session median dropped (V5.1). Lower is better.'],
    ['Short Run Pace', 'late_run_avg from practice_sessions Ã¢ÂÂ short-stint laps, mock-qual stints excluded. Lower is better.'],
    ['Start Position', 'qualifying_position from practice_sessions (placeholder until qual runs). Lower is better.'],
    ['Tire Falloff',   'trend_slope from practice_sessions Ã¢ÂÂ lap-time slope vs lap # in longest stint (min 10 laps required, else null Ã¢ÂÂ 50). Lower is better.'],
  ...
  return (
    <div className="card" style={{ marginBottom: 20 }}>
      ...
      <p style={{ fontSize: '0.8125rem', color: 'var(--text-muted)', marginBottom: 16 }}>
        Read-only reference Ã¢ÂÂ current weights and data sources used by Race Simulation.
      </p>
      ...
      <p style={{ fontSize: '0.75rem', color: 'var(--text-muted)', marginTop: 12, marginBottom: 0 }}>
        ...
        Road course auto-detected by track name Ã¢ÂÂ weights switch automatically in Simulation Center.
      </p>
    </div>
  )
}
