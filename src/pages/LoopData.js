    ...
    async function load() {
      try {
        ...
        const { data: trackData, error: trackErr } = await supabase
          .from('loop_data')
          .select('driver_name, year, finish_position, start_position, avg_position, driver_rating, quality_passes, pass_diff, pct_laps_led, pct_top15_laps, fastest_laps, laps_led, stage1_finish, stage2_finish')
