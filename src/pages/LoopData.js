...
{row.car_number && ((series === 'cup' || series === 'oreilly') ? <img src={(series === 'cup' ? '/car-numbers/' : '/car-numbers-oreilly/') + (({'133':'33'})[String(row.car_number)] || row.car_number) + '.png'} alt={'#' + row.car_number} style={{ height: 28, marginRight: 6, verticalAlign: 'middle' }} onError={(e)=>{e.target.style.display='none'}} /> : <span style={{ marginRight: 6, fontFamily: 'var(--font-mono)', fontSize: '0.88rem', color: 'var(--text-muted)', fontWeight: 600 }}>{'#' + row.car_number}</span>)}
<span
